...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
    ...
    public static final String DATE_FORMAT1 = "yy-MM-dd HH:mm:ss";
    ...
    /**
     * String content include Chinese ?
     */
    public static boolean isHaveChinese(String string) {
        if (!hasValue(string))
            return false;
        return (string.getBytes().length == string.length()) ? false:true ;
    ...
    /**
     * generate Query part eg. list contains "colA" "colB" "colB" "colD" ->
     * ('colA','colB','colB','colD')
     */
    public static String formInQuerySQL(List<? extends Object> listItems) {
        StringBuffer inSQL = new StringBuffer();
        inSQL.append("(");
        for (int i = 0; i < listItems.size() - 1; i++) {
            inSQL.append("'").append(listItems.get(i).toString()).append("',");
        }
        inSQL.append("'").append(listItems.get(listItems.size() - 1)).append(
                "'");
        inSQL.append(")");
        return inSQL.toString();
    }

    public static Object sqlDateConvert(Class type, Object value) {
    ...
    /**
     * add "where" or "or" sql part
     *
     */
    public static String formWhereSQL(boolean bFirst, String condition) {
        StringBuffer sHQL = new StringBuffer();
        if (bFirst) {
            sHQL.append(" where ").append(condition);
        } else {
            sHQL.append(" and ").append(condition);
        }
    ...
    public static String birthDayToAge(String birthDay) {
        ...
        String curryDay = DateUtils.getNowString();
        int i = Integer.parseInt(curryDay.substring(0, 4))
    ...
    /**
     * 校验字符串是否是Date类型yyyy-MM-dd HH:mm:ss
     * @author  lee
     * @param str
     * @return
     */
