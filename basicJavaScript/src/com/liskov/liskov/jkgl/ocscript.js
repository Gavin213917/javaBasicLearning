function imenus_data0() {
	...
	this.main_pointer_image = 'resource/js/arrow_down.gif'
	...
	this.sub_pointer_image = 'resource/js/arrow_right.gif'
...
function imenus_add_pointer_image(obj, dto, level) {
	...
	if ((level == 0) && (c_horizontal)) {
		...
		x3.setAttribute("fxoff", x25(x27_pointer(x4 + "_pointer_image_offx",
						dto, id)));
		x3.setAttribute("sloff", x25(x27_pointer(x4 + "_pointer_image_offy",
						dto, id)));
	}
	wht = "";
	...
	obj.onmousemove = function(e) {
		...
		if (lc.getAttribute("ish")) {
			...
			if (window.dp_zoomc)
				npos = dp_zoomc(npos);
			setTimeout("imenus_pointer_move('" + lc.id + "'," + npos + ",'h')",
					0);
		} else {
...
function x6(id, dto) {
	...
	sd += x18
			+ " li ul{"
			+ ((!window.imenus_drag_evts && window.name != "hta" && ulm_ie)
