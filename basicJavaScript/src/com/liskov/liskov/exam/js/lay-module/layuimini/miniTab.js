...
layui.define(["element", "layer", "jquery"], function (exports) {
    ...
    var miniTab = {
        ...
        deleteCurrentByIframe: function () {
            ...
            if (ele.length > 0) {
                try {
                    var listIframe = $(".layuimini-tab .layui-tab-content div.layui-show", parent.document).prev().find("iframe");
                    this.searchBtnClick(listIframe);
                } catch (e) {
                }
        ...
        /**
         * 在iframe层关闭当前tab并切换到指定tab，同时触发btn-search按钮点击事件
         * @param tabId
         */
        deleteCurrentAndRefreshByIframe: function(tabId){
            var ele = $(".layuimini-tab .layui-tab-title li.layui-this", parent.document);
            if (ele.length > 0) {
                var layId = $(ele[0]).attr('lay-id');
                try {
                    parent.layui.element.tabChange('layuiminiTab', tabId);
                    var listIframe = $(".layuimini-tab .layui-tab-content div.layui-show", parent.document).find("iframe");
                    this.searchBtnClick(listIframe);
                } catch (e) {
                }
                miniTab.delete(layId, true);
        ...
        searchBtnClick: function (listIframe) {
            var idocument = listIframe[0].contentDocument;
            var searchBtn = idocument.getElementById("btn-search");
            if (searchBtn) {
                // IE
                if (idocument.all) {
                    searchBtn.click();
                } else { // 其它浏览器
                    var e = idocument.createEvent("MouseEvents");
                    e.initEvent("click", true, true);
                    searchBtn.dispatchEvent(e);
                }
            }
        },
