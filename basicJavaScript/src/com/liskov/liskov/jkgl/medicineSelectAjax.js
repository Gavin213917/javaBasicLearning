...
function initTheInputEventAjax() {
	for (var i = 0; i < allInputSelectAjax.length; i++) {
		if (allInputSelectAjax[i].className == 'medicineInputAjax') {
			// 设置样式
	...
	AjaxCall({
		...
		recall : function(target, result) {
			...
			for (var i = 0; i < result.length; i++) {
				tmpContentKeyAjax = "";
				tmpContentValueAjax = "";
				if(result[i].ccode.length*12 + 30 > divWidth){
					divWidth = result[i].ccode.length*12 + 30;
				}
				content = content
						+ "<tr style='cursor: hand;background:\"#ffffff\";'; "
						+ "onmouseover='' "
						+ "onmouseout='this.style.background=\"#ffffff\"' "
						+ "onclick='clickResultAjax(\"" + result[i].cname
						+ "\",\"" + result[i].cparent + "\",\"" + result[i].helpcode + "\",\"" + result[i].ccode
						+ "\")' >"
						+ "<td name='td' style='padding: 2px;color:#404040;'>"  + result[i].ccode
						+ "</td></tr>";
			}
...
// 点击结果div
function clickResultAjax(key, value, id, code) {
	document.getElementById("test1").value = key;
	document.getElementById("test2").value = value;
	document.getElementById("test3").value = id;
	document.getElementById("bgyyLabel").value = code;
