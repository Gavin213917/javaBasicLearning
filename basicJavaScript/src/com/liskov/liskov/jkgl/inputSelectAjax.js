...
function initTheInputEventAjax() {
	for (var i = 0; i < allInputSelectAjax.length; i++) {
		if (allInputSelectAjax[i].className == 'selectInputAjax') {
			//取得模糊查询控件的CODE
			var Oldcode = document.getElementById(allInputSelectAjax[i].id.substring(0,allInputSelectAjax[i].id.indexOf('Label'))).value;
			// 设置样式（若CODE为空或名称为空）
			if (allInputSelectAjax[i].value == ""
					|| allInputSelectAjax[i].value == null||Oldcode==""||Oldcode==null) {
	...
	AjaxCall({
		...
		recall : function(target, result) {
			...
			for (var i = 0; i < result.length; i++) {
				...
				if(displayName.length*12 + 30 > divWidth){
					divWidth = displayName.length*12 + 30;
				}
				content = content
						...
						+ "onclick='clickResultAjax(\"" + code
						+ "\",\"" + name + "\",\"" + target.id
						+ "\");' >" + "<td name='td' style='padding: 2px;color:#404040;'></td>"
