...
function _481(_482,data){
	...
	if(data.total==0){
		if(typeof defaultHaveScrollForDatagrid=='function'){
			defaultHaveScrollForDatagrid($(_482).attr("id"));
			return;
		}
	}
	//end-wangsanfei
...
if(_485.length){
	if(data.total==1){
		if(data.rows[0].isScrollbarData){
			_485.pagination({total:0});
			return;
		}
	}
...
//libing--begin
	if(data.result=='99'){
		...
		if(content== null || content == ''){
			content =data.exceptionName;
		}
		var con="系统出现下述错误,请与系统管理员联系!<br/>";
		$.messager.alert('错误信息',con +content,'error');
	}else if(data.result=='TIMEOUT'){
		$.messager.alert('错误信息',data.exceptionMessage,'error',function(){
			window.location.href="/Gwcdc/sessionout.jsp";
		});
	}
...
//-------------------------------------------
var aa = $(":input").index(this);
for(ai=aa;ai<$(":input").length;ai++) {
	if($(":input")[ai].readOnly){
		continue;
	}
	if($(":input")[ai].id != null && $(":input")[ai].id !=''){
		var clsName = $(":input")[ai].className;
		if(clsName != null && clsName != '' && (clsName.indexOf('easyui-datebox')>=0 || clsName.indexOf('easyui-combotree')>=0 || clsName.indexOf('easyui-combobox')>=0)) {
			$(":input")[ai+1].focus();
		} else {
			$(":input")[ai].focus();
		}
